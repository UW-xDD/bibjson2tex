...
for item in bib:
    ...
    #TYPE OF DOCUMENT
    if isinstance(item['type'],unicode):
        typ=item['type'].encode('ascii','ignore')
    else:
        typ=item['type']
        
    #VOLUME NUMBER    
    if 'volume' in item.keys():    
        if isinstance(item['volume'],unicode):
            volume=item['volume'].encode('ascii','ignore')
        else:
            volume=item['volume']
    else:
        volume=''
    ...
    if 'number' in item.keys():    
        if isinstance(item['number'],unicode):
            number=item['number'].encode('ascii','ignore')
        else:
            number=item['number']
    else:
        number=''
    ...
    if 'pages' in item.keys():    
        if isinstance(item['pages'],unicode):
            pages=item['pages'].encode('ascii','ignore')
        else:
            pages=item['pages']
    else:
        pages=''
    ...
    if 'year' in item.keys():    
        if isinstance(item['year'],unicode):
            year=item['year'].encode('ascii','ignore')
        else:
            year=item['year']
    else:
        year=''
    ...
    if 'author' in item.keys():
        for name in item['author']:
            name['name']=name['name'].encode('ascii','ignore')
    ...
    if 'publisher' in item.keys():    
        if isinstance(item['publisher'],unicode):
            publisher=item['publisher'].encode('ascii','ignore')
...
with open('./output/bibtk.bib', 'wb') as f1:
    f1.write('\n'.join(biblist))
    
#PRINT THE BBL STRING
with open('./output/cite.bbl', 'wb') as f2:
